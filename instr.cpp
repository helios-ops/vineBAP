...
void Instruction::decode() {
    ...
    prog = vine::byte_insn_to_asmp(vine::asmir_arch_x64, address, rawbytes, size);
    ...
    // printf("now try to decode : vine_ir size = %d\n", statements.size());
    /*
    for ( statements_t::iterator iter = statements.begin(); 
	  iter != statements.end(); 
	  iter ++
	)
    */
    for ( std::vector<vine::Stmt *>::iterator iter = block->vine_ir->begin();
          iter != block->vine_ir->end();
	  iter ++
       	)
    {
	// printf("ins-%lx\n", address);
	// printf("stmt type: %d\n", (*iter)->stmt_type);
	char * tmpstr = (*iter)->to_strptr();

	// std::cout << "cout str : " << strlen(tmpstr) << "\n";
	printf("%s\n", tmpstr);
	free(tmpstr);
