...
static const char *func_unique_name(Function *func) {
	...
	snprintf(buf, sizeof(buf), "unknown_0x%lx", func->getAddress());
	return buf;
    } else {
	return name;
    }
...
InterProcCFG::copy_nodes_to_ipcfg(Function *func) {
    ...
    if (0 && exits.size() != 1) {
	fprintf(stderr, "Function %s (%lx) has %d exits\n",
	...
	// 某个函数内的基本块 —— IPCFG 的一个节点
	IPCFGNode n = bb_to_node[bb] = add_vertex(ipcfg);
	bb_ptr[n] = bb;
	pth[n] = -1;

	// subgraphs: < 函数名称，函数的所有IPCFG中的节点 >
...
InterProcCFG::make_call_ret_edges(Instruction *i, bool include_returns) {
	...
	// 在 caller-bb 和 callee-entry 之间建立 CALL_EDGE 边
	tie(e, okay) = add_edge( bb_to_node[callee_entry], 
			         bb_to_node[bb], // caller-bb 
				 ipcfg
			       );
	assert(okay);
    ...
    // 找不到可能的 callee，就在 caller-block 和其后继节点之间建立一个边（CALL_TO_RET_EDGE）
    if (!seen_callees) 
    {
	// None of the callees exist in our representation, so add
	// a call-to-ret edge unconditionally.
	debug2( "Adding call-to-ret %lx --> %lx\n", 
		bb->getAddress(), 
		bb_next->getAddress()
	      );

	tie(e, okay) = add_edge( bb_to_node[bb_next],  // caller's successor
			         bb_to_node[bb],       // caller
				 ipcfg
			       );
	assert(okay);
...
/* copy_edges_to_ipcfg: 将 func 函数内的所有的边拷贝到 ipcfg 中。
 * 这里值得注意的是，拷贝到 ipcfg 里的边，与原始 func 函数内的边是源末节点完全颠倒的!
 * */
void
InterProcCFG::copy_edges_to_ipcfg( Function * func,
				   bool       include_returns,      // 是否在 ipcfg 中，在 callee-exit 和 caller-retsite 之间添加边
				   bool       include_call_to_rets, // 是否在 ipcfg 中，在 call-block 和其后继之间添加边
				   bool       include_calls         // actually nonsense ! 
				 ) 
{
    property_map<IPCFGraph, edge_type_t>::type edge_type
	= get(edge_type_t(), ipcfg);
	...
	{
	    tie(e, okay) = add_edge( bb_to_node[bb2], // the successor of the caller block
			             bb_to_node[bb1], // the caller block 
				     ipcfg
			           );
	    assert(okay);
	    edge_type[e] = CALL_TO_RET_EDGE;
	...
	{
	    tie(e, okay) = add_edge( bb_to_node[bb2], // original target-bb 
			             bb_to_node[bb1], // original source-bb
				     ipcfg
				   );
	    assert(okay);
	    int weight;
	    
	    // bb1 的结构首部是 bb2 : 这条边是回边
...
void InterProcCFG::set_target_warning(Warning *w,
	...
	if (leader == cfg->getEntry()) {
	    //fprintf(stderr, "BB 0x%lx is not in an (intra-proc) loop\n",
	    //    slice_bb->getAddress());
	} else {
	    //fprintf(stderr, "BB 0x%lx is in a loop with head 0x%lx\n",
...
InterProcCFG::InterProcCFG(map<addr_t, Function *> &functions)
{
	...
	copy_edges_to_ipcfg( func, 
			     true,  // include_returns      : 是否在 ipcfg 中，在 callee-exit 和 caller-retsite 之间添加边
			     true,  // include_call_to_rets : 是否在 ipcfg 中，在 call-block 和其后继之间添加边
			     true   // include_calls        : actually nonsense !
			   );
...
/* CALL_EDGE <src, dst>: 
 * dst = caller's callsite
 * src = callee's entry site
*/
void InterProcCFG::remove_call_edges_from_g1() {
	...
	{
	    debug2( "Removing call edge %lx --> %lx\n", 
		    caller->getAddress(),
		    callee->getAddress()
		  );
...
void InterProcCFG::remove_ret_edges_from_g2() {
	...
	if (edge_type[*ei] == RETURN_EDGE && IS_G2(src) && IS_G2(dst)) {
	    debug2("Removing return edge %lx --> %lx\n", src->getAddress(),
		   dst->getAddress());
	    toremove.push_back(*ei);
	}
    }
...
/* 在 g1 图的 caller-bb 和对应的 g2 图的 caller-bb 之间，建立一个边
 * */
						  ...
						  ) 
{
    ...
    graph_traits<IPCFGraph>::edge_iterator ei, ei_end;
    for (tie(ei, ei_end) = edges(ipcfg); ei != ei_end; ++ei) {
	BasicBlock *src = bb_ptr[target(*ei, ipcfg)]; // the caller bb
	BasicBlock *dst = bb_ptr[source(*ei, ipcfg)]; // caller-bb's successor
	
	// Add an edge between the callsite in g1 and the callsite in g2
	if (edge_type[*ei] == CALL_TO_RET_EDGE) {
	    assert(src->isCall());

	    debug2( "Found call-to-ret edge %lx --> %lx\n", 
		    src->getAddress(),
		    dst->getAddress()
		  );
	    ...
	    for ( cit = src->getCfg()->call_targets_begin(*src);
		  cit != src->getCfg()->call_targets_end(*src); 
		  ++cit
		) 
	    {
		...
		debug2( "Creating call-to-ret edge %lx --> %lx (%s %lld)\n", 
		        src->getAddress(), 
		        dst->getAddress(), 
		        f->getName(),
		        costs[f]
	...
	debug2( "Creating call-to-call edge %lx --> %lx (0)\n", 
	        src->getAddress(), 
		dst->getAddress()
	      );
	tie(e, okay) = add_edge( bb_to_node[dst], // g2 图中对应的 caller-bb 
			         bb_to_node[src], // g1 图中的 caller-bb
				 ipcfg
			       );
	assert(okay);
	edge_weight[e] = 0;
...
IntraProcCFG::IntraProcCFG(Function *f) {
    ...
    copy_edges_to_ipcfg( func, 
		         false, // include_returns      : 是否在 ipcfg 中，在 callee-exit 和 caller-retsite 之间添加边
			 true,  // include_call_to_rets : 是否在 ipcfg 中，在 call-block 和其后继之间添加边
			 false  // include_calls        : actually nonsense !
		       );
}
...
void IntraProcCFG::add_calls_costs_to_edges(costmap &costs) {
	...
	if (edge_type[*ei] == CALL_TO_RET_EDGE) {
	    ...
	    debug2("Assigning cost %lld to edge %lx --> %lx\n", mincost,
...
void compute_functions_costs(CallGraph *cg, costmap &costs) {
    ...
    for (vector<Function *>::const_iterator it = ordering.begin();
	 it != ordering.end(); ++it) {
	debug2("Processing function %lx %s\n", (*it)->getAddress(),
	       (*it)->getName());
