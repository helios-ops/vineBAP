...
private:
    ...
    /* boost 序列化的一般代码如下：
       ...
       类 Section 的 serialize 函数，将在类 Section 的一个对象 obj 和 Archive 类的对象 ss 进行 ss << obj（序列化）或 ss >> obj (反序列化)
       时被调用（序列化和反序列化也可以用 & 表示）。

       注意这里的 serialize 函数，其第一个参数 Archive 可承载 ifstream 和 ofstream 两个类的实例。
       当为 std::ofstream 时，serialize 函数即完成 “序列化输出” 的功能；
       当为 std::ifstream 时，serialize 函数即完成 "反序列化输入" 的功能。
     */
