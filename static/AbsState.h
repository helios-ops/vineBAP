...
// functor.
template <class T>
inline typename boost::intrusive_ptr<Region<T> > AbsState<T>::find(int id) {
...
inline boost::intrusive_ptr<ValueSet<T> > AbsState<T>::read(AlocPair p){
    ...
    assert( RegionType::weak2strong(regionToReadCurrVersion->getRegionType()) == RegionType::weak2strong(regionToRead->getRegionType()) 
...
inline BOOSTPTR AbsState<T>::write(AlocPair p, VSetPtr v) {
    ...
    RegionPtr writeToRegionCurrVersion = find(writeToRegion->getId());
    assert( writeToRegionCurrVersion.get() && 
            "Couldn't find region with the same type-id."
	  );
    assert( RegionType::weak2strong(writeToRegionCurrVersion->getRegionType()) == RegionType::weak2strong(writeToRegion->getRegionType())
            && "Region type mismatch."
	  );

    RegionPtr newVersion = writeToRegionCurrVersion->write( writeToAddress, 
	    						    v
							  );
...
template <typename T>
inline boost::intrusive_ptr<ValueSet<T> > AbsState<T>::read(VSetPtr adr) {
    VSetPtr v = ValSetTy::getTop();
    for ( typename ValSetTy::const_iterator I = adr->begin(), E = adr->end(); 
	  I !=E; 
	  ++I
	)
    {
