...
public:
	    ...
	    ) 
	{
        ...
        for ( defuse_map_t::const_iterator it = uses.begin(); 
	      it != uses.end(); 
              ++it
	    ) 
	{
    ...
    void slice(Instruction *start, std::set<Instruction *> &instrs) {
        ...
        while (!wlist.empty()) {
            ...
            if (definitions.find(aloc) != definitions.end()) {
                for ( it = definitions[aloc].begin(); 
		      it != definitions[aloc].end(); 
		      ++it
		    ) 
		{
...
void VSAInterpreter<T>::visitXallocInstr(Instruction &i, const char *a) {
    ...
    for ( typename ValSetTy::const_iterator I = vs_size->begin(), E = vs_size->end(); 
	  I !=E; 
	  ++I
	)
    {
	...
	/* 这里值得注意几个点：
	 * 1. 这里的 heap_objects 的类型为 std::map<BasicBlock *, RegionPtr>，可见分析过程中假定一个 bb 内只有一个分配的堆对象
	 *   （或者说，一个 bb 内只有一个 call malloc 程序点）。究其缘由，要注意到这里 cfg 起初是通过 PIN 得到，在分析过程中会
	 *    进行 bb 的 split；
	 * 
	 * 2. 这里的静态分析，并非以函数摘要的形式对一个函数进行完一次分析后就完事，而是在每次函数被调用时候，都进入到这个函数   
	 *    内部进行分析。由此导致函数内部的同一个程序点可能在程序静态分析的过程中被多次分析到。call malloc 程序点亦不例外。
	 *    
	 * 3. 在对 call malloc 程序点的静态分析过程中，需要为本次 “分配内存的动作” 构造相应的 abstract region。具体的构造过程遵循
	 *    如下原则：
	 *    (1) 当第一次进入这个程序点时，构造一个 strongly updatable region；
	 *    (2) 如果这个程序点在随后又被分析到，则构造一个 weakly updatable region。之后如果再次分析到该程序点，则继续使用这个
	 *        weakly updatable region。
	 *    由此可见，对于一个 alloc site，至多会有两个抽象 region 和它相关联。
	 *    
	 *    XXX: strong-update VS weak-update
	 *    A strong update overwrites the contents of the written region, and represents a definite change;
	 *    A weak update computes a join of the old and new contents.
	 *
	 *    之于为什么这样设计，可以考虑如下程序:
	 *    void func() {
	 *        while (1) {
        ...
        if (_Base::heap_objects.find(bb) == _Base::heap_objects.end()) 
	{
	    ...
	    const int id = AbsRegion::getNextId();
            _Base::cur_state = _Base::cur_state->addNewRegion( absdomain::rg::STRONG_HEAP, 
                                      			       I->get().second
							     );
