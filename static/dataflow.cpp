...
std::string def_to_string(def_t vd) {
  char tmp[24];
  sprintf(tmp, "@%lx", vd.second->getAddress());
  return vd.first + tmp;
}
...
void computeGeneratedVariables( Instruction  * i, 
				defuse_map_t & defmap, // [IN] : std::map< Instruction *, std::set<var_t> >  : 指令的所有定值 
                                def_set_t    & defs    // [OUT]: std::set< std::pair<var_t, Instruction *> > : 每个定值所在的指令
			      ) 
{
    ...
    {
	def_t d = def_t(*it2, i);
	debug4( "Instruction %lx/%lx defines %s\n", 
		i->getAddress(), 
	     	i->getBasicBlock()->getAddress(), 
		def_to_string(d).c_str()
	      );
	defs.insert(d);
    }
}
...
void computeGeneratedVariables( BasicBlock   * bb, 
				defuse_map_t & defmap, 
                                def_set_t    & defs
			      ) 
{
...
void computeGeneratedVariables( Cfg          * cfg, 
				defuse_map_t & defmap, 
                                def_set_t    & defs
			      ) 
{
    for ( Cfg::const_bb_iterator it = cfg->bb_begin(); 
    	  it != cfg->bb_end(); 
    	  ++it
      	) 
    {
...
void computeKilledVariables( BasicBlock   * bb, 
			     defuse_map_t & defmap, // std::map< Instruction *, std::set<string> > : 指令的所有定值 
                             def_set_t    & defs,   // std::set< std::pair<var_t, Instruction *> > : 每个定值所在的指令
			     def_set_t    & kills
			   ) 
{
    def_set_t tmp;
    computeGeneratedVariables( bb, 
		               defmap, 
			       tmp
		             );

    for ( def_set_t::const_iterator it = defs.begin(); 
	  it != defs.end();
          ++it
	) 
    {
	    ...
	    ) 
	{
	    ...
	    {
		debug4( "BasicBlock %lx kills %s %d\n", 
			bb->getAddress(), 
			def_to_string(*it).c_str(), 
			kills.find(*it) == kills.end()
		      );
		kills.insert(*it);
	    }
	}
...
void computeReachingDef( Cfg          * cfg, 
	                 defuse_map_t & defmap,   // std::map< Instruction *, std::set<var_t> > 
                         bb2def_set_t & reachdefs // std::map< BasicBlock *, std::set< std::pair<var_t, Instruction *> > > 
		       ) 
{
    ...
    computeGeneratedVariables( cfg, 
	    		       defmap, 
			       defs
	    		     );

    // 计算每个 bb 的 gens 和 kills
	    ...
	    ) 
	{
	    ...
	    // 与本轮次比较。如果有变化，即说明未收敛。TODO: 这里仅仅关注于集合的 size，是否不够充分？
...
void computeUseDefChain( Cfg          * cfg, 
	                 defuse_map_t & defmap, // [IN] 
			 defuse_map_t & usemap, // [OUT]
                         ud_chain_t   & ud_chain
		       ) 
{
    ...
    computeReachingDef( cfg, 
	                defmap,   // [IN]
			reachdefs
		      );
    debug2( "Reaching definitions computed for %lx\n", 
      ...
      ) 
  {
	...
	) 
    {
      ...
      for (var_set_t::const_iterator dit = defmap[i].begin(); 
        ...
        debug4("Found local definition for %s @ %lx\n", dit->c_str(), 
               i->getAddress());
        loc_defs[*dit] = i;
      }
    }
...
void computeSlice( Instruction             * i, 
	           defuse_map_t            & defmap, 
		   defuse_map_t            & usemap,
		 ...
		 ) 
{
    ...
    debug2( "Slicing %lx [%lx %s]\n", 
	    i->getAddress(), 
	    func->getAddress(), 
            func->getName()
	  );
  
    computeUseDefChain( cfg, 
	    		defmap, 
			usemap,
		       	ud_chain
		      );
	...
	debug3("  Processing instruction %lx\n", i->getAddress());
    ...
    {
	debug2(" %lx", (*it)->getAddress());
    }
  
    debug2("\n");
    debug2("Pending definitions:");
    for ( var_set_t::const_iterator it = notdone.begin(); 
	  it != notdone.end();
          ++it
	) 
    {
