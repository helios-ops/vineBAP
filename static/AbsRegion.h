...
/* ValMap := RegionID  -> SInterval
 * Region := SInterval -> ValMap    指定了该 Region 内部的所有 offset 对应的取值集合 
 * 
 */
template <typename T>
...
public:
    ...
    RegionPtr getWeaklyUpdatable() {
        ...
        return isStronglyUpdatable() ?  RegionPtr( new Region( *this, 
						               strong2weak(getRegionType())
							     )
						 ) : RegionPtr(this);
    }
    ...
    RegionPtr getStronglyUpdatable() {
        // Register region is always strongly updatable
        return isWeaklyUpdatable() ? RegionPtr( new Region( *this, 
							    weak2strong(getRegionType())
							  )
					      ) : RegionPtr(this);
    }
    ...
    bool isRegister() const { 
        return regty == rg::WEAK_REGISTER || regty == rg::STRONG_REGISTER; 
    }
    bool isStack() const { 
...
inline bool Region<T>::checkWrite(TPtr p, VSetPtr vs) {
    for ( typename ValueSet<T>::const_iterator I = vs->begin(), E = vs->end(); 
	  I != E; 
	  ++I
	) 
    {
...
template <typename T>
inline typename Region<T>::RegionPtr Region<T>::getFresh( rg::RegionTy       ty, 
							  StridedIntervalPtr size
							) 
{
    int i = 0;
...
template <typename T>
inline typename Region<T>::RegionPtr Region<T>::write( TPtr    p, // address: strided interval
						       VSetPtr s
						     )
{
...
handle_misalignment:
    ...
    /* std::transform(lb, ub, out, op) : 对于 [lb, ub] 范围内的元素，依次应用 op 算符，最终将结果存储到 out 开始的范围内。
     *     这里的 lb、ub、out 都是 iterator 类型。
     * */
    std::transform( overlapping.begin(), 
		    overlapping.end(),
                    std::back_inserter(intervals),
                    utils::select1st<ContentPair>() // 获取 pair 中的第一个元素。这里是表征地址的 StridedInterval
		  );
    p->split(intervals, out);

    // Erase all overlapping intervals
    for ( content_iterator I = overlapping.begin(), E = overlapping.end(); 
	  I != E; 
	  ++I
	) 
    {
...
template <typename T>
inline typename Region<T>::RegionPtr Region<T>::write( TPtr     p, 
						       AlocPair ap
						     ) 
{
