...
{
    return get( std::make_pair( Region<T>::getFresh(), 
			        T::get(x, s) // 这里调用的是 StridedInterval::get 函数，一般情况下返回 (x, x+s, s) 
			      )
	      );
}
...
inline typename ValueSet<T>::VSetPtr ValueSet<T>::join(_Self& p) {
    ...
    const_iterator TI = begin(), TE = end(), OI = p.begin(), OE = p.end(); 
    for (; TI != TE && OI != OE; ) {
    ...
    // 要求 join 的结果，能够以预期的偏序 “包含” this 和 p。
...
inline typename ValueSet<T>::VSetPtr ValueSet<T>::meet(_Self& p) {
    ...
    for ( const_iterator TI = begin(), TE = end(), OI = p.begin(), OE = p.end(); 
	  TI != TE && OI != OE; 
	) 
    {
        AlocPair tp = TI->get();
...
inline bool ValueSet<T>::withinBoundsOf(const _Self& a) const {
    ...
    const_iterator TI = begin(), TE = end(), AI = a.begin(), AE = a.end();
    utils::Cmp<RegionPtr> cmp;
						       ...
						       ) 
{
        ...
        for (; AI != AE; ++AI) 
	{
            ...
            vs = vs->join( *get( std::make_pair( tp.first->isGlobal() ? ap.first : tp.first,
                                                 ((*tp.second).*op)(*ap.second) // C++ 成员函数指针 TPtr (T::*OperatorTy)(T&)
					       )
			       )
			 );
        }
