...
int main(int argc, char **argv) {
    ...
    unserialize( vm["cfg"].as<string>().c_str(), 
		 prog, 
		 callgraph, 
		 functions
	       );

    prog.createSection( Prog::getDefaultArgvAddress(), 
	    		".argv"
		      );
...
#undef MAGICFOO
	...
	{
	    // 通过命令行参数 function，指定静态分析的起始函数 main 
	    addr_t f = strtoul( vm["function"].as<string>().c_str(), 
			        0, 
				16
		              );
