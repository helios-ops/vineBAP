...
StridedIntervalPtr StridedInterval::get( int      low, 
					 int      high, 
					 unsigned stride
				       ) 
{
...
StridedIntervalPtr StridedInterval::get( int      x, 
					 unsigned s  // 默认为 1
				       ) 
{
...
StridedIntervalPtr StridedInterval::meet(_Self& sip) {
    ...
    {
        return get( max(smaller->lo, larger->lo), 
		    min(smaller->hi, larger->hi), 
		    lcm(smaller->strd, larger->strd) // 两个步长的最小公倍数
		  );
    }
}
...
// this 在 a 之内（这里未考虑 stride 的情况，所以约束关系弱于 subsumes ！）
...
void StridedInterval::split( const SIPtrVector & a, 
	                     const SIPtrVector & b,
                             SIPNVector        & out
			   ) 
{
